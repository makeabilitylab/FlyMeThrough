...
* Our depth data is generated by **Depth-Pro** and stored in the `depth.npz` format.  
  If you prefer to use your own depth data like in `depth.png` format, please modify `diamserver/edit_segdep.py`, which is responsible for combining the segmentation data from **SAM2** and the depth data from **Depth-Pro**.  
  You should adjust this module to correctly load and integrate your custom depth data into the pipeline.
